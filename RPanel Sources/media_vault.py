...
# ================== SERVER LOGIC ==================
def get_files_list():
    if not os.path.exists(VAULT_FOLDER): return []
    files = []
...
def api_files():
    ...
    resp = jsonify({"files": get_files_list(), "inbox_count": inbox_count, "tags": load_tags()})
        ...
        for f in os.listdir(INBOX_FOLDER):
            if f.lower().endswith(ALLOWED_EXTENSIONS):
                ...
                if os.path.isfile(filepath):
                    ...
                    with open(os.path.join(VAULT_FOLDER, f + '.enc'), 'wb') as enc_file: enc_file.write(enc_data)
                    os.remove(filepath)
                    imported += 1
...
def api_upload():
    ...
    for file in request.files.getlist('files[]'):
        ...
        if ext in ALLOWED_EXTENSIONS:
            data = file.read()
            enc_data = fernet.encrypt(data)
            with open(os.path.join(VAULT_FOLDER, filename + '.enc'), 'wb') as f:
                f.write(enc_data)
            uploaded += 1
...
def api_delete():
    ...
    enc_filepath = os.path.join(VAULT_FOLDER, filename + ".enc")
    if os.path.exists(enc_filepath):
        os.remove(enc_filepath)
        # Удаляем тэги удаленного файла
        tags = load_tags()
        if filename in tags:
            del tags[filename]
            save_tags(tags)
...
def api_change_password():
    ...
    if success:
        global CURRENT_KEY
        with open(MASTER_KEY_FILE, 'rb') as f: enc_master = f.read()
        CURRENT_KEY = Fernet(derive_key(new_pass)).decrypt(enc_master)
...
.dropbtn { background: var(--surface); border: none; color: var(--text); padding: 8px 15px; border-radius: 8px; cursor: pointer; font-size: 13px; font-weight: 500; display: flex; align-items: center; gap: 8px; transition: background 0.2s; }
.dropbtn:hover { background: var(--surface-hover); }
.dropdown-content { display: none; position: absolute; top: 45px; left: 0; background: var(--surface); min-width: 180px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); z-index: 101; border-radius: 12px; padding: 6px; border: 1px solid rgba(255,255,255,0.1); }
body.light-theme .dropdown-content { border: 1px solid rgba(0,0,0,0.1); }
.dropdown-content.show { display: block; }
.dropdown-item { display: flex; align-items: center; padding: 10px; border-radius: 8px; cursor: pointer; font-size: 14px; color: var(--text); transition: background 0.2s; user-select: none; }
.dropdown-item:hover { background: var(--surface-hover); }
.dropdown-item input { margin-right: 10px; width: 16px; height: 16px; accent-color: var(--accent); cursor: pointer; }

.search-wrapper { display: flex; align-items: center; background: var(--surface); border-radius: 8px; border: 1px solid transparent; transition: border-color 0.2s, width 0.3s; width: 150px; }
...
<div id="top-bar">
    <div class="top-left">
        ...
        <div class="search-wrapper">
            ...
            <input type="text" id="search-input" class="search-input" placeholder="Search tags..." oninput="handleSearch()">
        </div>
    </div>
    <div class="top-right">
...
<script>
let files = []; let fileTags = {}; let currentIndex = 0; let scale = 1, translateX = 0, translateY = 0; 
...
function toggleFilterDropdown() { document.getElementById("filter-content").classList.toggle("show"); }
window.onclick = function(event) {
    if (!event.target.matches('.dropbtn') && !event.target.closest('#filter-dropdown')) {
        const dropdown = document.getElementById("filter-content");
...
function closeViewer() { 
    ...
    document.body.classList.remove('hidden-ui'); // ФИКС БАГА: Возвращаем UI
    document.getElementById('tag-overlay').classList.remove('show'); // Скрываем панель тэгов
...
function renderTagList(tags) {
    ...
    list.innerHTML = '';
    if (tags.length === 0) {
        list.innerHTML = '<span style="color: var(--text-dim); font-size: 12px;">No tags yet</span>';
        return;
    }
    tags.forEach(tag => {
...
function saveTags() {
    ...
    fetch('/api/tags/update', {
        ...
        body: JSON.stringify({filename: file, tags: tags})
    })
    .then(r => r.json())
    .then(d => {
        if(d.success) {
...
document.addEventListener('keydown', (e) => {
    // Игнорируем нажатия, если фокус в поле ввода (поиск, тэги, настройки)
